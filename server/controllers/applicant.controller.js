...
export const generateAICV = async (req, res, next) => {
  try {
    ...
    const personalInfo = {
      ...(existing.personalInfo && typeof existing.personalInfo === "object"
        ? existing.personalInfo
        : {}),
      ...(body.personalInfo && typeof body.personalInfo === "object" ? body.personalInfo : {}),
    };
    const experience =
      body.experience !== undefined ? body.experience : existing.experience || [];
    ...
    // Call OpenAI GPT-4o from merged profile + user form input (falls back to local logic)
    ...
    const summaryFromForm =
      body.summary !== undefined
        ? body.summary
        : body.personalInfo?.bio !== undefined
          ? body.personalInfo.bio
          : undefined;
    const draftSummary =
      summaryFromForm !== undefined
        ? summaryFromForm
        : existing.summary || personalInfo.bio || "";

    const data = {
      ...existing,
      personalInfo: Object.keys(personalInfo).length > 0 ? personalInfo : existing.personalInfo,
      experience,
      skills,
      education,
      certifications,
