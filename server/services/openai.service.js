...
export async function generateCVSummary(profile, cvData = {}) {
  const pi = cvData.personalInfo && typeof cvData.personalInfo === "object" ? cvData.personalInfo : {};
  const fullNameFromForm = String(pi.fullName || "").trim();
  const fullName =
    fullNameFromForm || `${profile.firstName || ""} ${profile.lastName || ""}`.trim() || "Candidate";
  const email = pi.email || "";
  const phone = pi.phone || profile.phone || "Not provided";
  const locationOrNationality = pi.location || profile.nationality || "Filipino";
...
Nationality / location: ${locationOrNationality}
Phone: ${phone}

Work Experience:
${experience.length > 0 ? experience.map((e) => `- ${e.position || e.title} at ${e.company || e.employer || "—"} (${e.startDate || "?"} – ${e.endDate || "Present"}): ${e.description || "No description"}`).join("\n") : "No work experience listed"}

Skills: ${skills.length > 0 ? skills.join(", ") : "None listed"}

Education:
${education.length > 0 ? education.map((e) => `- ${e.degree} from ${e.institution || e.school || "—"} (${e.year || ""})`).join("\n") : "None listed"}
...
function buildLocalSummary(
  ...
  if (nationalityLabel) s += ` from ${nationalityLabel}`;
